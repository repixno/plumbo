<?php
   ...
   /**
    * Enumerate user's shared albums
    * 
    *
    */
   ...
   class APIAlbumsSharedBy extends JSONPage implements IValidatedView {
      ...
      /**
       * Returns a list of albums shared by user
       * 
       * @api-name albums.sharedby
       * @api-auth required
       * @api-javascript yes
      ...
      public function Execute( $limit = 10 ) {
         ...
         //todo: implement ordertype, orderby
         //SELECT bildealbum.aid FROM tilgangtilalbum_dedikert left join bildealbum on tilgangtilalbum_dedikert.aid=bildealbum.aid WHERE bildealbum.aid IN ( SELECT aid FROM bildealbum WHERE uid = 736073 AND deleted_at IS NULL ) order by bildealbum.year desc ;
         ...
         try {
            ...
            $res = array_reverse( DB::query( sprintf( 'SELECT bildealbum.aid FROM tilgangtilalbum_dedikert left join bildealbum on tilgangtilalbum_dedikert.aid=bildealbum.aid WHERE bildealbum.aid IN ( SELECT aid FROM bildealbum WHERE uid = ? AND deleted_at IS NULL ) order by %s', $sortby ), Login::userid() )->fetchAll() );
