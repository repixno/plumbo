...
config( 'website.storage' );
   ...
   class Streamthumb extends Webpage implements IView {
      ...
      //public $watermark  = '/var/www/repix/sites/website/views/images/watermarks/reedfoto-logo-kvit-trans20.png';
      public $watermark  = '/var/www/repix/sites/website/views/images/watermarks/reedfoto_trans20.png';
      ...
      public function image($bid, $width=0, $height=0, $bw = 0, $rotate = null ){
         ...
         $filesrc = $imagespath .  $image->getFilename();
            ...
            try{
               if($image->hashcodeaaa && file_exists($cachefilename)){
                  session_write_close();
                  ...
                  if( $image->licensefee > 0 || in_array( $image['owner_uid'],  $this->projectowners ) ){
                     ...
                     $watermark->rotateImage(new ImagickPixel('#00000000'), -35);
                     $watermark->thumbnailImage( (int)  $thumb->getImageWidth()  , $thumb->getImageHeight()  , true );
                     $wx = ( $thumb->getImageWidth() -  $watermark->getImageWidth()  )  / 2;
                     $wy = ( $thumb->getImageHeight() -  $watermark->getImageHeight() ) / 2;
                     
                     $thumb->compositeImage( $watermark, $watermark->getImageCompose() , $wx , $wy );
                  }
                  if( $bw == 1 ){
