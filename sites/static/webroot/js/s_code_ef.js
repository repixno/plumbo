...
var s_account="cewe-eurofoto-no"
...
s.linkInternalFilters="javascript:,as.photoprintit.de,mueller.de/foto,as.photoprintit.com,cs.photoprintit.com,cs.photoprintit.de,cewedigital.lan,asp.photoprintit.de/faq"
...
/* External Campaigns */
	if(!s.campaign)
		s.campaign=s.getQueryParam('CREF');
		s.campaign=s.getValOnce(s.campaign,'s_CREF',0);
...
/*Time Parting 15min*/
s.prop11=s.eVar11=s.getTimeParting('h','0','2012');

/*copy props to evars*/
if(s.prop6){
	s.eVar9=s.prop6;
	s.eVar9=s.getValOnce(s.prop6,'s_c6',0);
}
if(s.prop9){
	s.eVar8=s.prop9;
	s.eVar8=s.getValOnce(s.prop9,'s_c9',0);
}
/*product selection stacking*/
s.eVar13=s.crossVisitParticipation(s.eVar12,'s_ev13','90','5','>','purchase',0);

/*Page percent viewed*/
var ppvArray = s.getPercentPageViewed(s.pageName);
s.prop12 = ppvArray[0] //contains the previous page name
s.prop13 = ppvArray[1] //contains the total percent viewed
	
	/* To send the hit on an exit link*/
	var exit_link = s.exitLinkHandler()
		if(exit_link) {
...
s.visitorNamespace="cewecolor"
s.trackingServer="cewecolor.112.2o7.net"
