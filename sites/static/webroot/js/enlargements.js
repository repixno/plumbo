...
function pickUploadedImage( image ) {
	var newimage = new Image();
	newimage.src = image.screensize;
	$(newimage).load( function() {
		$('#cropbox, #previewimage').attr('src', image.screensize );
...
function updateOnChange(printtype){
   ...
   if(printtype == 2.5){
      var selectedStr = $('#canvas2cm').val();	      
   }
   else if (printtype == 4){
      var selectedStr = $('#canvas4cm').val();	
   }
   else if(printtype == "2x2"){
      var selectedStr = $('#gallery2x2').val();
   }
   else if(printtype == "1x3"){
      var selectedStr = $('#gallery1x3').val();
   }
...
function save( debug ) {
	...
	page = {};
	page.giftQuantity = $('#gift-quantity').val();
	page.productoptionid = currentProductOptionId;
	/*page.productid = $('#productid').val();*/
	page.size = 'small';
	page.redeye = false;
	page.varnish = false;
