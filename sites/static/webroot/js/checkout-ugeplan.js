var ajaxLoader = '<img src="http://a.static.repix.no/gfx/gui/ajax-loader.gif" class="loader"/>';
...
$(document).ready( function() {
   ...
   $('#contact-email').focus();
			...
			success: function(msg) {
				...
				if( msg.result != false) {
				   ...
				   //location.reload();
					...
					$('#password').after('<label class="inputError">Forkert adgangskode</label>');
				}
			},
			error: function(msg) {
				$('#password').next('.loader').remove();
	...
	function writePaymentOptions( data ) {
	   ...
	   var paymentwrapper = $( '<div class="span-12 last"></div>' );
	   ...
	   $.each( poptions, function( i, option ) {
	      ...
	      tmpwrapper.append( '<label class="large" for="payment_' + option.id + '">' + option.title + '<span class="quiet"> DKK <span class="price hide" id="paymentprice_' + option.refid + '">' + option.price + '</span><span>' + option.price + '</span></span></label>' );
	...
	function writeDeliveryOptions( data ) {
	   ...
	   $.each( deliveryoptions, function( i, option ) {
	      ...
	      var sub2 = $( '<div class="span-3"><span class="large"> DKK <span class="price hide" tal:content="' + option.price + '"></span><span id="deliveryprice_' + option.refid + '"> ' + option.price + ' </span></span></div>' );
      ...
      $.post( '/api/1.0/checkout/paymentmethods', { 
         ...
         if( data.result ) {
            ...
            $.each( poptions, function( i, option ) {
               ...
               tmpwrapper.append( '<label class="large" for="payment_' + option.id + '">' + option.title +
                                    '<span class="quiet"> DKK <span class="price hide" id="paymentprice_' +
                                    option.refid + '">' + option.price + '</span><span>' +
                                    option.price + '</span></span></label>' );
               paymentwrapper.append( tmpwrapper );
            });
	...
	$( '#delivery-methods-wrapper :radio' ).click( function() { 
      ...
      $.post( '/api/1.0/checkout/paymentmethods', { 
         ...
         if( data.result ) {
            ...
            var poptions = data.paymentoptions;
            $.each( poptions, function( i, option ) {
               var tmpwrapper = $( '<div class=""><div class="col-md-8"/></div>' );
                              
               var input = $( '<input type="radio" name="payment-method" value="' + option.refid + '" id="payment_' + option.id + '" />' );
               if( option.isPreset ) {
                  input.attr( 'checked', true );
                  tmpwrapper.find('div').append( input );
                  updatePaymentCost( option.price );
               } else {
                  tmpwrapper.find('div').append( input );
                  updatePaymentCostFromElement( input );
               }
               tmpwrapper.find('div').append( '<label class="large" for="payment_' + option.id + '">' + option.title +
                                 '<span class="quiet"> DKK <span class="price hide" id="paymentprice_' +
                                 option.refid + '">' + option.price + '</span><span>' + option.price +
                                 '</span></span></label>' );
               
               if( option.refid  == '407' ){
                    tmpwrapper.append( '<div class="span-4 last"><img src="http://a.static.repix.no/cms/images/dankort.jpg" /></div>' );
               }
               paymentwrapper.append( tmpwrapper );
