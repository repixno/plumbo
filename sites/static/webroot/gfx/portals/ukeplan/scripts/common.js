...
$(document).ready( function() {
    $('input.quantity').before('<a href="#" class="btn btn-default changequantity hidden-xs">-</a>').prev().click( function() { 
        if ( parseInt( $(this).next().attr('value') ) <= 0) { return false };
        $(this).next().attr('value', parseInt( $(this).next().attr('value') ) - 1);
        $(this).next().trigger('change');
        return false;
    });
    $('input.quantity').after('<a href="#" class="btn btn-default changequantity hidden-xs">+</a>').next().click( function() { 
