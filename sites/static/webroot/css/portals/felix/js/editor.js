...
(function($) {
	...
	var FelixEditor = function(element, options) {
		...
		$('.choosetemplate input').on('click', function() {
			_this.changeTemplate($(this).data('src'), $(this).val(), $(this).data('size'));
		});
		...
		function load(src, color, size) {
			...
			//_this.changeTemplate( src, color, size );
