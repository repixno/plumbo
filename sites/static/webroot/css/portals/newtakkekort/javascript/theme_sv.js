            ...
            $.post( "/api/1.0/takkekort/cewecards", {catid:catid, count: cewecount, group: id, articleid: cewearticle[id] } ,function( data ) {
                	...
                	cardcontainer.append(
                							 $('<li/>').append(
                								$('<a/>', { href: arr[i].purchaseurl,id: arr[i].id, class: "cewe_examplethumb"}).append( 
                									...
                									$('<p/>', {html:  arr[i].title +"<br><span id=\"price\">från " +  arr[i].price + "</span> per st." })
                							 )
                							 );
                    cewecountarray[id]++;
                }
                ...
                if( message == "continue" ){
                    ...
                    $('<li/>', {class: "cmore"} ).append(
                       $('<h2/>', { text: "Visa fler", class:"showmorecewe", id:id   })
                       )
                    );
                }else {
                    var articleid =  arr[0].articleid;
                    var cewecardurl = "https://as.photoprintit.com/web/85021449/views/productEditor.jsf?productId=" + articleid + "&openDesigns=true";
                    if( catid == 458 ){
                        cewecardurl = cewecardurl + "&categoryId=category-christmas";
                    }
                    if( message != 'stop' ){
						$(cardcontainer).append(
							$('<li/>', {class: "cmore"} ).append($('<h2/>', { text: "Visa färre", class:"showlesscewe", id:id})
							));
					}
                    $(cardcontainer).append(
                        $('<li/>', {class: "showall"} ).append($('<h2/>').append( $("<a/>", {text:"Visa alla", href: cewecardurl }))
                        ));
                }
            });
        }
                	...
                	cardcontainer.append(
                							 $('<li/>').append(
                								$('<a/>', { href: arr[i].purchaseurl, id: arr[i].id, class: "ef_examplethumb" }).append( 
                									...
                									$('<p/>', {html:  arr[i].title +"<br><span id=\"price\">från " + formatPrice( arr[i].price ) + "</span> per st." })
                							 )
                							 );
                efcountarray[id]++;
                }
		...
		function removeEf( id ){
            ...
            $(cardcontainer).append(
                    $('<li/>', {class: "cmore"} ).append(
                       $('<h2/>', { text: "Visa flera", class:"showmoreef", id:id   })
                       )
            );
        }