      ...
      public function Execute( $orderid = 0 ) {
         ...
         if( isset( $orderid ) ) {
            ...
            if( Dispatcher::getPortal() == 'SKA-001' ){
               ...
               $lopenummer = DB::query( 'SELECT lopenummer FROM historie_mal WHERE ordrenr = ?' , $orderid )->fetchSingle();
               $stabburetid = DB::query( 'SELECT printtype FROM mal_order WHERE malorderid = ? ', $lopenummer )->fetchSingle();
               $stabburetid = explode( '_', $stabburetid );
               #$id = DB::query( 'SELECT thumbid FROM leverpostei_order WHERE id = ?',$stabburetid[1] )->fetchSingle();
         
               
               $this->lokkid = "3214qerwfqwretqe45rqw3e12";
