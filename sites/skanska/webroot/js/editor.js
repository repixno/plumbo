	...
	// 2019 var text = new Array();
// 2019	var textgroup = new fabric.Group( );
// 2019	var yeargroup = new fabric.Group( );
	...
	$( function(){
		...
		if (!isCanvasSupported()){
			$('.editorbox').append('Editoren st&oslash;ttes dessverre ikke av din nettleser. Du kan laste ned en gratis nettleser, vi anbefaler Google Chrome.<br/>\
		...
		$(document).on( 'click', '#download', function(){
			...
			$.post('/api/1.0/skanska/stabburetthumb', {
			    name: name,
			    image: thumb
			    }, function (response) {
	...
	function ChangeImageSrc( name ){
		...
		var newsrc = "/images/stream/image/" + name  + '/400/480';
		fabric.Image.fromURL(newsrc , function(img) {			
			userimg = img.set({
			//  left: 155,
			 // top: 175,
			  // flytter sjølve bildet
			   left: 275,
			  top: 315,
			  lockUniScaling : true,
			  id: name
    ...
    this.save = function(){
        ...
        if( userimg.id  == "preimage" ){
			...
			//var thumb = canvas.toDataURL();
          // 2019 var names = $('#name').val();
          // 2019  var year = $('#year').val();
			...
			$.post('/api/1.0/skanska/save', {
				...
				window.location.href = '/stickers/checkout/bestilling';
			}, 'json');
            return false;
		}
    }
	...
	function shareThumb( media ){
		...
		var sharelink = '';
		if( media == 'facebook' ){
			sharelink = 'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('http://stabburetbeta.repix.no/bestill/ShareFacebook/');
		}

		$.post('/api/1.0/skanska/savethumb', {
		    thumb: thumb
		    }, function (response) {
			var u = 'http://stabburetbeta.repix.no/bestill/ShareFacebook/' + response.data;
        ...
        $.post(
            ...
            function(response){
                setTimeout(function() {
					...
					$('.loadingInfo').html('<div><h3>Lokket er nå klart til å kunne deles</h3></div>\
										   <div><a class="btn large" href="'
										   + sharelink + u +
										   '" onclick="window.open(this.href, \'mywin\', \'left=20,top=20,width=500,height=500,toolbar=1,resizable=0\'); hideloader(); return false;" >\
											<img src="http://static.repix.no/cms/images/del-facebook-2.jpg"/></span>Del lokket</a></div>');
	...
	function upenurl(href) {
        ...
        window.open(href, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=100, width=400, height=400");
