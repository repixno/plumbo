   ...
   class ExecuteKlarna extends WebPage implements IView {
      ...
      public function Execute() {
       ...
       //$ksarray = explode( '/', $checkoutId );
        ...
        //mail('tor.inge@eurofoto.no', "klarna" , $klarna->order['status'] . " ".  $checkoutId . "\n orderid: ". serialize( $klarna->order )  );
            ...
            if( $orderid ){
               ...
               try{
                  if( date( strtotime( $complatedate  . " +4 hours" ) < date('Y-m-d H:i:s') )){
                     mail('tor.inge@eurofoto.no', "Klarna not OK" , $complatedate . " ------------\n " .   serialize( $klarna->order) );
                  }
               }Catch(Exception $e ){
                   mail('tor.inge@eurofoto.no', "Klarna error" , $e->getMessage() );
