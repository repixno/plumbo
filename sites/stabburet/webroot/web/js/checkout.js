...
$(document).ready( function() {
	...
	$('form#contact-and-delivery-info').validate({
		...
		messages: {
			cname: {
				required: "Må utfylles",
				fullname: "Skriv inn fornavn og etternavn"
			},
			cemail: {
				required: "Må utfylles",
				email: "Din e-postadresse må være i formatet navn@domene.no"
			},
			czipcode: {
				required: "Må utfylles",	
				minlength: "Fyll inn fire sifre"	
			},
			ccity: {
				required: "Må utfylles"	
			},
			...
			'repeat-password': {
				required: "Må utfylles"	,
				equalTo: "Oops, Passordene dine stemmer ikke."
			},
			eula: {
				required: "Du må lese og godta vilkårene og betingelsene"
			},
			delivertostore:{
			...
			phone:{
			   ...
			   digits: "Kan kun inneholde tall",
			   minlength: "Telefonummer må inneholde 8 siffer"
			}
		    }
	    });
    ...
    $(document).on('click', '#checkout-login-button',  function() {
            ...
            /*$.post('/api/1.0/user/login', function(data) {
                console.log( data );
                return false;
            });*/
                    ...
                    success: function(msg) {
                            if( msg.result != false) {
                               ...
                               //location.reload();
                                ...
                                $('.loader').remove();
                                $('#password').after('<label class="inputError">Feil passord</label>');
                            }
                    },
                    error: function(msg) {
	...
	$( '#delivery-methods-wrapper :radio' ).click( function() { 
	   ...
	   $( '#payment-methods-wrapper' ).empty();
	   $( '#payment-methods-wrapper' ).append( '<img style="display:block; margin-left:auto; margin-right:auto; margin-top: 20px;" src="htts://static.repix.no/gfx/gui/ajax-loader.gif" />' );
