   ...
   class EditMediaclip extends AdminPage implements IView {
      ...
      public function Execute($id) {
        ...
        $efcustomer = array(
			...
			'location' => "https://julie.repix.no",
			'orderid' => $id,
			'unique' => $unique->id,
			'predefinert' => 0,
			'server' => "julie.repix.no",
			'adminuser' => "1",
		);
	...
	//	$designers = "flash,html5";
		...
		// $this->flashurl = 'http://bente.repix.no/ECommerceBridge/designer/
		 /*$this->flashurl = 'http://bente.repix.no/ECommerceBridge/designer/
			...
			if( $art_type == 'Photobook' && strpos( $project_xml, 'xmlns:photobook="Mediaclip.Photobook.Model' ) ){
				$module = "Photobook";
				
			}
			
			
			elseif ( $module == 'Gifting' && strpos( $project_xml, 'xmlns:gifting="Mediaclip.Gifting.Model' ) ){
				$module = "Gifting";
			}
			...
			$this->flashurl = 'https://bente.repix.no/ECommerceBridge/designer
									?module=' . $module .
									//'&themeUrl=' . $themeUrl .
									'&culture=nb-NO' .
									'&adminuser=' . $logindata['userid'] .
