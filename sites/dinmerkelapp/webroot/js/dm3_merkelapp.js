 function update() {
    ...
    $('img#preview').parent().append( '<div id="imageloader" style="width:308px; height:131px"><img src="http://a.static.repix.no/gfx/gui/ajax-loader-gray.gif"/></div>' );
    ...
    if( type == 'fargelapp' ){
     ...
     $.post('/api/1.0/merkelapp/clipart', {
         ...
         $(response.images).each(function (index, value) {
             $('#clipart').append('<img id="clip" title="' + value + '"src="' + staticroot + 'gfx/merkelapp/' + folder + '/' + category + '/' + value + '" width="60px" style="margin-right: 5px;"/>');
         })
     }, 'json');
 ...
 function getBackgrounds() {
     ...
     $.post('/api/1.0/merkelapp/backgrounds', {
        backgroundcat: backgroundcat,
         ...
         $(response.images).each(function (index, value) {
             $('#background').append('<img id="backgroundfile" title="' + value + '"src="' + staticroot + 'gfx/merkelapp/backgroundfiles/' + backgroundcat + '/thumbs/' + value + '" style="width: 60px; margin-right: 5px;"/>');
         })
     }, 'json');
