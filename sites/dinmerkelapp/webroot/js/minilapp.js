 function update() {
    ...
    var color = encodeURIComponent( $('input[name="font-color"]').val() );
     ...
     var previewlink = '/api/1.0/minilapp/preview?projectid=' + projectid + '&line1=' + line1 + '&line2=' + line2 + '&line3=' + line3 + '&font=' + $('select[name$="font-type"]').val() + '&gravity=' + gravity + '&image=' + ownimage + '&clipart=' + clipart + '&backgroundfile=' + backgroundfile + '&color=' + color + '&treshold=' + treshhold + '&crop=' + selection + '&colormode=' + colormode;
 ...
 $(document).ready(function () {
     ...
     $('.clips').each( function(){
      ...
      $.post('/api/1.0/merkelapp/clipart', {
         ...
         $(response.images).each(function (index, value) {
             THIS.append('<img class="clip"  data-category="'+ category +'" title="' + value + '"src="' + staticroot + 'gfx/merkelapp/' + folder + '/' + category + '/' + value + '" width="50px;" style="margin-right: 5px;"/>');
         })
     }, 'json');
     ...
     $('.backgrounds').each( function(){
         ...
         var category =  $(this).data('cat');
          ...
          $.post('/api/1.0/minilapp/backgrounds', {
            backgroundcat: category,
            }, function (response) {
                $(response.images).each(function (index, value) {
                     //console.log(value);
                  THIS.append('<img class="backgroundfile" data-cat="' + category + '" title="' + value + '"src="' + staticroot + 'gfx/minilapp/backgroundfiles/' + category + '/thumbs/' + value + '" style="width: 50px; margin-right: 5px;"/>');
                })
         }, 'json');
