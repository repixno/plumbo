<?php
   ...
   class CartDefault extends Webpage implements IView {
      ...
      protected $template = 'betalingsvalg.index';
      ...
      public function coupon() {
         ...
         if ( !empty( $discountCode ) ) {
            ...
            $discount = DiscountCampaign::fromCode( $discountCode );
            //$res = $discount->getDiscounts( $discountCode );
            
            $currenttime = date('Y-m-d H:i:s');            
            $res = DB::query( "SELECT * FROM discount_campaigns AS dc WHERE dc.type =  2 AND dc.code=? AND dc.active=1 AND dc.stop> ? AND dc.start < ?", $discountCode, $currenttime, $currenttime )->fetchAll(2);
            $res = $res[0];
            
            $campaign = new DiscountCampaign( $res['id'] );
            $res['info'] = $campaign->asArray();
            $res['products'] = DiscountCampaign::enumRefId( $res['id'] );
            ...
            $discount = $cart->addCartDiscountDM( $res );
            $cart->save();
