<?php
   ...
   class GiftCreator extends WebPage implements IView {
      ...
      public function Broderingpreview( $font = null, $text = ''){
         
         $imgewidth = 300;
         $imageheight = 150;
         
         $text = $text ? " " . $text . " " : "Brodering";
         
         $font = $font ? $font : 'ar-berkley';
         
         
         
         $fontarray = array(
            'ar-berkley' => 'ARBERKLEY.ttf',
         ...
         $this->template = null;
         $folder = '/var/www/repix/sites/website/webroot/fonts/';
         
         $image = new Imagick();
         $draw = new ImagickDraw();
         
         $image->newImage($imgewidth, $imageheight, new ImagickPixel('white'));
         
         
         
         $pixel = new ImagickPixel( 'black' );
         /* Font properties */
         $draw->setFont( $folder . $fontarray[$font] );
         $draw->setFontSize( 40 );
         ...
         $textimage->newImage($metrics['textWidth'] + 10, $metrics['textHeight'] + 10, 'white');
         $textimage->setImageFormat('jpg');
         $textimage->drawImage($draw);
         
         $textimage->rotateImage ( 'white',  -18 );
         
         $tx = $textimage->getImageWidth();
         $ty = $textimage->getImageHeight();
         
         if( $tx > $imgewidth || $ty > $imageheight ){
            $textimage->resizeImage( $imgewidth , $imageheight, Imagick::FILTER_LANCZOS,1 );
            $tx = $imgewidth;
            $ty = $imageheight;
         }
         ...
         $x = ( $imgewidth / 2 ) - ( $tx / 2  );
         $y = ( $imageheight / 2 ) - ( $ty / 2  );
         
         
         
         $image->compositeImage($textimage, $textimage->getImageCompose(), $x, $y); 
         
         /* Create text */
         ///$image->annotateImage($draw, $x, 300, 0, $text );
