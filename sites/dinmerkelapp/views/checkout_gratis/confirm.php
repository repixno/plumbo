<?php
   ...
   class CheckoutConfirm extends WebPage implements IView {
      ...
      protected $template = 'checkout_gratis.confirm';
      ...
      public function Execute() {
         ...
         if( $user ){
            //$userexist = DB::query('SELECT * FROM brukar WHERE brukarnamn = ? AND logingroup =?', $email, 'GRATISDINMERKELAPP' )->fetchAll();
            $hasorders = DB::query('SELECT * FROM historie_ordre WHERE uid = ?',  $user->uid )->fetchAll();
      
            
            /*Denne er unødvendig vist du er kom so lang veit en allereie at det finns en bruker 
             *if( !$userexist ){
               relocate( '/' );
               exit;   
            }*/
            ...
            if( $hasorders ){
               relocate( '/eksisterende' );
               exit;   
            }
            
      /*     if( $nopass == true ) {
            if( !User::registered( $email ) ) {
               Session::pipe( 'registererror', "User is not registered" );
               relocate( '/sss' );
               die();
            }
         }*/
         ...
         if( $userinfo['coupon'] ){
            ...
            $cart = $this->coupon( $userinfo['coupon'] , $cart );
         }
      ...
      public function coupon( $discountCode, $cart  ) {
