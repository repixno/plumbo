<?php
   ...
   class Checkout extends Webpage implements IView {
      ...
      public function Execute(){
         ...
         $hasorders = DB::query('SELECT * FROM historie_ordre WHERE uid = ?', $this->user['userid']  )->fetchAll();
         
         
         if( $hasorders ){
         //   Util::Debug("du kan kun bestill 1 gratis odree blblblblbllalblfadlladflal" );
         //  Util::Debug( $hasorders );
         ...
         $itemsarray = $cart->asArray();
         
         $itemsarray = $itemsarray['items']['merkelapp'];
         ...
         if( is_array( $items ) ){
            foreach( $items as $key=>$item ){
               $cart->removeItem($item['prodno'], $key );
               $cart->save();
