...
var helperheight;
	...
	$(document).ready( function() {
		...
		$('#confirm').on('hidden.bs.modal', function (e) {
			document.location.href = '/create/banderoll/';
		});
		...
		$('#skygge').on( 'change', function(){
			...
			if( $(this).prop('checked') ){
				$('.textholder span').css( 'text-shadow', '#333 2px 2px 5px' );
			}else{
				$('.textholder span').css( 'text-shadow', '' );
			}
		...
		$(document).on('click','.template-thumb,#choose-template',  function() {
			...
			if( textvars.text1.fontfamily == "Comic Sans MS"){
				fontfile = "comic.ttf";
			}else if( textvars.text1.fontfamily == "Monotype Corsiva"){
				fontfile = "MTCORSVA.ttf";
			}
			else if( textvars.text1.fontfamily == "Arial"){
				fontfile = "arial.ttf";
			}
			else if( textvars.text1.fontfamily == "Times New Roman"){
				fontfile = "times.ttf";
			}
			else if( textvars.text1.fontfamily == "Verdana"){
				fontfile = "verdana.ttf";
			}
			
			$('#font-list span').css('font-family', textvars.text1.fontfamily ).text(textvars.text1.fontfamily).data('font', fontfile );
			...
			$(templateImage).load( function() {
				...
				$('.text2').css({
					'top' : textvars.text2.top
					})
				$('.text2 span').data('fontsize', textvars.text2.fontsize );
				$('.text2 span').css({
					'font-size': textvars.text2.fontsize,
					'color': textvars.text2.color,
					'font-family' : textvars.text2.fontfamily
					});
				
				$("#sp-light input").spectrum("set", textvars.text2.color );
				stretchToFit();
				...
				while(  $('.text2 span').width() > 320  ){
					var fontsize = parseInt( $('.text2 span').css('font-size') ) - 1 ;
					$('.text2 span').css('font-size', fontsize + 'px');	
				}
			...
			if( $(this).val() > 0 ){
				...
				if( productid == 4134 ){
					...
					$('.textholder').each( function(i) {
						...
						var fontspan = $("#font-list").find('span');
						...
						text.font = fontspan.data('font');
						
						
						
						if( $('#skygge').prop('checked') ){
							text.shadow = textspan.css('text-shadow');
						}
