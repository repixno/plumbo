   ...
   function pickImage(image) {
      ...
      function pickFigur(image) {
        //console.log(image);
        
        var newimage = new Image();
        
        var $bildeboks = $('#' + selectedcropbox ).parent();
        
        if( $bildeboks.hasClass('cropFrame') ){
                $bildeboks = $bildeboks.parent(); 
        }
        originalimage[image.id] = image;
        ...
        $(newimage).load(function () {
                ...
                $velgknapp = $('<div id="'+selectedcropbox+'" class="clipselect"><button class="btn btn-default">Velg bilde</button></div>');
                $velgknapp.css( 'width' , $bildeboks.width() + "px" );
                /*if(  $('#' + selectedcropbox ).attr('imageid') == 'noimage' ){
                        $('#' + selectedcropbox ).parent().html('<img id="' + selectedcropbox + '" class="cropbox" imageid="" src=""/>');
                }*/
                
                var imagewidth = newimage.width / newimage.height * $bildeboks.height();
                
                $(newimage).attr( {"id" : selectedcropbox, "class" : "figur", "imageid" : "noimage", "height" : $bildeboks.height(), "width" : imagewidth });
                //newimage.width = 'auto';
                newimage.height = $bildeboks.height();
                //$bildeboks.css('border-color', 'transparent');
                $('.imageplaceholder').css('border-color', 'transparent');
                //$('#imageframe').val('off');
                $bildeboks.append(newimage);
                
                $bildeboks.append( $velgknapp );
                $velgknapp.hide();

           
       });
       
       newimage.src = image.data('figur');
   ...
   $(document).ready(function () {
       ...
       $(document).on( 'click', '.openimagebox', function(){
        ...
        $(document).on( 'click', '.clipselect', function(){
            ///console.log($(this).parent().parent().find('.cropbox').attr('id'));
        
            selectedcropbox = $(this).parent().find('.figur').attr('id');
            $('#imageselect').modal();
        });
           ...
           if ($('input[name=clipart]:checked').val() == 'dinner') {
               ...
               $('.grid').each(function (index, item) {
                   if ($('.grid').size() - 1 == index) {
                       ...
                       $('table.grid  td').height(dinnerheight);
...
$(document).on( 'click', '.rotateLeft', function(){
      ...
      $('#background').on( 'change', function(){
        ...
        $(document).on( 'click', '.clipartdiv' ,function(){
                
                var cat = $(this).data('cat');
            
                $.get( "/create/clipart/catlist/" + cat , function(catlist){
                    
                     $(".clipartcontainer").slideUp( "slow", function() {
                        var text = "";            
                        $('.ClipartPicker').show( 'fast', function(){
                            
                            for (i = 0; i < catlist.length; i++) {
                                $('.pickcontainer').append(
                                        $('<a href="#" class="clipartpicker" data-figur="/create/clipart/' + cat + '/' + catlist[i] +'" id="' + catlist[i] + '"></a>' ).css(  { 'background-image' : 'url(/create/clipart/' + cat + '/' + catlist[i] + ')' } ) );
                            }
        ...
        $(document).on('mouseenter', '.figur', function(){
                console.log("mouseenter");
                $(this).parent().find('.clipselect').show();
        });
        
        $(document).on('mouseleave', '.imagecontainer', function(){
                //console.log("mouseleave");
                $(this).parent().find('.clipselect').hide();
         
         });
        
        
        $(document).on( 'click', '.clipartpicker', function(){
                var image = $(this);
                pickFigur( image );
        ...
        $('.tilbake').on('click', function(){
                ...
                $('.ClipartPicker').slideUp( "slow", function() {
                        $(".clipartcontainer").show();
                        $('.pickcontainer').html('');
                });
