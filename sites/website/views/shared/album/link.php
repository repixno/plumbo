<?php
   ...
   class SharedAlbumLink extends WebPage implements IValidatedView {
      ...
      public function Execute( $aid = 0, $albumname = '', $page = 1, $publickey = '' ) {
         ...
         $this->mediarss = sprintf( '%s/shared/album/link/rss/%d/%s/%d/%s', WebsiteHelper::rootBaseUrl(), $aid, $albumname, $page, $publickey );
      ...
      private function fetchPaginatedAlbums( $aid = 0, $page = 1, $publickey = '' ) {
         ...
         if( $aid > 0 ) {
            ...
            try {
               ...
               $pages = ceil( count( $imagelist ) / SharedAlbumLink::$paginationsize );
      		   $images = array_splice( $imagelist, ( $page - 1 ) * SharedAlbumLink::$paginationsize, SharedAlbumLink::$paginationsize );
      		   ...
      		   $this->pagination = array(
                  'current' => $page,
      		      ...
      		      'last' => $pages,
      		      'items' => array(
                     'from' => ( ( $page - 1 ) * SharedAlbumLink::$paginationsize ) + 1,
                     'to' => ( ( $page - 1 ) * SharedAlbumLink::$paginationsize ) + count( $images ),
                     'count' => count( $images ),
      		      ),
      		   );
