    ...
    class MediaclipImportScript extends Script {
		...
		//Util::debug($this->projectXml);
            ...
            <calendar:holidayText text="2. påskedag" />
                    ...
                    $orgXmlId = $xmlData["id"];
                  //  $xmlId = $id . "-" . $order['product_id'] . "-" . $order['quantity'] . "-" . $order['user_id'];
                    
                     $xmlId = $order['id'] . "-" . $order['nfoto_artnumber'] . "-" . $order['nfoto_quantity'] . "-" . $order['user_id'];
			 ...
			 if(strpos($this->projectXml,"collage:photo")!==false){
                        ...
                        if( $error != 1 ) {
                            ...
                            try{
                                ...
                                file_put_contents( $this->savepath . "/nfoto-order.txt" , date( "Y-d-m H:i:s" ) );
                            }
                            catch (Exception $e){
                                echo $e->getMessage() . "\n";
                            }
                            // Update processed status of orders. Om det ikkje eksisterer mediaclip prosjekt vil ve
                        //    DB::query( "UPDATE mediaclip_projects SET processed=now() WHERE id = ?" , $order[ 'id' ] );
                        DB::query( "UPDATE mediaclip_projects SET nfoto_status=2   WHERE id = ?" , $order[ 'id' ] );
                        } else {
                            $error = 0;
                        }
                    }
