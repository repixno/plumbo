  ...
  /* $orderDownloadQuery = sprintf( "SELECT ordrenr, tidspunkt, source FROM historie_ordre WHERE 
      ...
      */
               ...
               //denne her kan du velge å importere spesifikk ordre
                  $orderDownloadQuery = sprintf( "SELECT ordrenr, tidspunkt, source FROM historie_ordre WHERE ordrenr in ( 2408216)" );
      ...
      public function fetchReedfotoImage( $imageid, $orderid, $id, $artikkelnr  ){
         ...
         if( !file_exists( sprintf( '/home/produksjon/webspool/Reedfoto_preview/%s', $orderid ) ) ){
            mkdir( sprintf( '/home/produksjon/webspool/Reedfoto_preview/%s', $orderid ), 0755 , true );
         }
         ...
         try{
            ...
            $savepath = sprintf( '/home/produksjon/webspool/Reedfoto_preview/%s/%s/autoedit', $orderid, $artikkelnr );
                 ...
                 $gallery = explode("x", $printtype);
                  ...
                  //if( file_exists( $this->imagepath."/".$filename ) ) {
                     ...
                     for( $i=0;$i<$n;$i++ ){
                        ...
                        if( $ordertemplate['artikkelnr'] == 7454 || $ordertemplate['artikkelnr'] == 7474 || $ordertemplate['artikkelnr'] == 7461 ){
                           
                           /*$oldfulldy = $fulldy;
                           $fulldy = $fulldy * 1.2;
                           $movetop = (  $fulldy - $oldfulldy ) / 2;
                           $fully = $fully - $movetop;*/
                           ...
                           if( $texts[$i]["rotate"] > 0 ){
                              ...
                              $ry = $fulldy;
                              $t= deg2rad( $degree );
                              $bx =  intval( $rx * cos($t) + $ry * sin($t) );
                              $by  = intval( $ry * cos($t) + $rx * sin($t) );
                              
                              $rotatemovey = ( $by - $fulldy ) / 2;
                              $fully = intval( $fully - $rotatemovey );
                              
                              $rotatemovex = ( $bx - $fulldx ) / 2;
                              $fullx = intval( $fullx - $rotatemovex );
                              
                              $rotatetext = "convert -background none  -rotate " . $texts[$i]["rotate"] . " text$unique.$counter.png text$unique.$counter.png";
                           }
                           
                           //Justere tekst studentskilt
                           if( $shadow ){
                              if( $fullx < 0 ) {
                              ...
                              if( $ordertemplate['artikkelnr'] == 7458 || $ordertemplate['artikkelnr'] == 7474 ){
                                 $text = " " . $text . " ";
                              }
