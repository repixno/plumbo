<?PHP
   ...
   class OrderImportScript extends Script {
     ...
     // public $smilFolder = '/home/produksjon/webspool/c8/Output/'; 
      ...
      Public function Main(){
         ...
         foreach ( $ready_orders  as $to_do_order ){
          ...
          //   $uidkunde = DB::query( "SELECT uid FROM historie_ordre WHERE ordrenr = ?", $to_do_order['order_id'] )->fetchSingle();
           ...
           //Outspool folder til stabburet
            $convert2 = sprintf( '%sOutspool/%s_%s_%s/', $this->stabburetFolder, $to_do_order['order_date'], $to_do_order['order_id'], $to_do_order['article_id'] );
            ...
            if( file_exists( $convert2 . 'Proceed.txt' ) || file_exists( $convert2 . 'proceed.txt' ) ){
               ...
               if( in_array(  $to_do_order['article_id'], $stabburet ) ){
                  ...
                  $conversions->save();
                  try{
                     Util::debug( $convert2 . 'autoedit' );
                     
                     if( !file_exists( $convert2 . 'autoedit' ) ){
                        mkdir( $convert2 . 'autoedit' );
                     }
                     
                     chdir($convert2 . 'autoedit' );
                     foreach ( glob( $convert2 . '*roceed.txt' ) as $filename ){
                        $this->StabburetLeverpostei( $filename, $org_source, $convert2, $to_do_order['order_id'] );
                     }
                  ...
                  try{      
                     ...
                     foreach ( glob( $convert2 . '*' ) as $preview_file ){
      ...
      private function StabburetLeverpostei( $filename, $org_source, $convert2, $orderid ){
         ...
         $white = new ImagickPixel('white');
          ...
          $customer  = DB::query( 'SELECT * from historie_kunde where ordrenr = ?' , $orderid  )->fetchAll( DB::FETCH_ASSOC  );
                     ...
                     $customer = $customer[0];
                        ...
                        $namedecode = utf8_decode( $customer['navn'] );
                        $adressdecode = utf8_decode( $customer['adresse1'] );
                        $steddecode = utf8_decode( $customer['sted'] );
         ...
         copy( '/home/produksjon/stabburet/maler/' . $templateOrder->malid . '.png' ,$convert2 .  "autoedit/" . $giftTemplate->fullsize_src );
         ...
         /*if( !file_exists( $org_source .  "autoedit/" . $giftTemplate->fullsize_src ) ){
            ...
            ssh2_scp_recv($connection, '/data/global/maler/orginal/' . $giftTemplate->fullsize_src , $convert2 .  "autoedit/" . $giftTemplate->fullsize_src );  
          ...
          $cutFilename = "/home/produksjon/skanska/kuttefil.pdf" ; 
             ...
             //  $hotfolder2 = "/home/produksjon/stabburet_hotfolder";
               ...
               if( $templateOrder->malid == 2553 ){
                  $size = 74.3;
                   $size2 = 90.4;
                  $cutFilename = "/home/produksjon/stabburet/kuttekanta/tynnkant_stabburet_2019c.pdf" ; 
               }else{
                  $size = 87.4;
                  $size2 = 105.4;
                  //stirt stabburet lokk
                  $cutFilename = "/home/produksjon/stabburet/kuttekanta/tynnkant_stabburet_2019_stor.pdf" ; 
               }
               //create pdf printfile
           //    $namer = utf8_decode( $customer['navn'] );
               
               
               
               
               
               $pdf = new FPDI( 'P', 'mm', array( $size, $size2 + 25 ) );
               $pdf->AddPage();
               ...
               $badcodetext = base_convert( $orderinfo['ordrenr'], 10, 36  ) . " " . $orderinfo['ordrenr'];
                  ...
                  $pdf->Code39(5, $size + 3,$badcodetext,0.8,11);
                  
                 //
                $pdf->SetFont('Arial','',9);
                   $pdf->Text(5, $size + 25,$namedecode,4.9,11);
                   $pdf->Text(5, $size + 31,$adressdecode,3.9,11);
                     $pdf->Text(5, $size + 37, $customer['postnr'] . " " . $steddecode,14.9,14);
                  
              // $badcodetext = base_convert( $orderinfo['ordrenr'], 10, 36  ) . " " . $orderinfo['ordrenr'];
               //$pdf->Code39(5, $size + 40,$badcodetext,0.8,11);
               
             
                  // $pdf->Write(1, $customer['postnr'] . $customer['sted']);
