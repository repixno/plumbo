   ...
   class ManualOrder{
            ...
            foreach( $collectionarray as $type => $items ) {
               ...
               foreach( $items as $prodno => $item ) {
                  ...
                  switch( $type ) {
                     ...
                     import( 'website.order.manual.handlers.merkelapp' );
                        
                     // Can have multiple templates under the same product
                     
                     if( count( $item ) > 0 ) {
                        foreach( $item as $merkelapp ) {
                           $handler = new ManualOrderHandlerMerkelapp();
      ...
      private function OrderItems(){
         ...
         if( is_array( $orderdetails['article']['gifts'] ) ){
          ...
          if( is_array( $orderdetails['article']['merkelapp'] ) ){
         
            foreach ( $orderdetails['article']['merkelapp'] as $prodno=>$gift ){
               
              $this->productType = 'merkelapp';
              
              $this->SetupMerkelapp( $prodno, $gift );
            }
         }
         ...
         if( $orderdetails['delivery'] == "jp" ){
            
            if( $orderdetails['deliverystore'] ){
              $this->items['deliverytype']['storeid'] = $orderdetails['deliverystore'];
            }
            
            $this->items['deliverytype']['refid'] = 484;
            $this->items['deliverytype']['price'] =  $orderdetails['deliveryprice'];
            $this->items['deliverytype']['weight'] = $this->total;
            $this->items['paymenttype']['price'] = 0.5;
            
         }
         else if( $orderdetails['delivery'] == 'local' ){
            $this->items['deliverytype']['refid'] = 484;
      ...
      private function setupMerkelapp( $prodno, $merkelapp){

         foreach( $merkelapp as $ret ){
      ...
      private function createEDI() {
         ...
         $dinfo = $this->deliveryinfo;
         $dtype = $this->deliverytype;
      ...
      private function insertContactAndDeliveryData() {
         ...
         $oc->deliveryemail = $this->user->email;
