...
<?php
   ...
   class ManualOrder{
      ...
      public function executeManualOrder( $data ) {
         ...
         $user = User::fromUsernameAndPortal($this->data['email'], 'RP-001');
         ...
         if( $user ){
             ...
             $user->setCellPhone( $userinfo["mobile_phone_number"] );
             $user->streetaddress = $userinfo['address'];
             $user->zipcode = $this->data['zipcode'];
      ...
      private function OrderItems(){
         ...
         if( is_array( $orderdetails['article']['gifts'] ) ){
			...
			$this->items['paymenttype']['price'] = $this->delivery;	
          $this->items['deliverytype']['price'] = $this->startprice;
      ...
      */
      ...
      private function insertContactAndDeliveryData() {
         ...
         $oc->userid = $this->userid;
			//$oc->userid = $item['customerId'];
			
			
			
         $oc->contactemail = $this->user->email;
         $oc->contactname = $this->data['fullname'];
         $oc->contactfirstaddress = $this->data['address'];
